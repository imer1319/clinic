            ...
            <table class="table">
                ...
                <tbody>
                    <tr v-for="specialty in filteredSpecialties" :key="specialty.id">
                        <td class="cursor-pointer" @click.prevent="show(specialty)">{{ specialty.description }}
                        </td>
...
export default {
    ...
    methods: {
        show(specialty) {
            this.form.id = specialty.id
            this.form.description = specialty.description
            this.form.status = specialty.status
        ...
        store() {
            axios.post('/api/specialties/store', this.form)
                .then(() => {
                    this.get();
                    this.form = {
                        id: '',
        ...
        update() {
            axios.put('/api/specialties/' + this.form.id, this.form)
                .then(() => {
                    this.get();
                    this.form = {
                        id: '',
    ...
    computed: {
        filteredSpecialties() {
            return this.specialties.filter(specialty => {
                return specialty.description.toLowerCase().includes(this.search.toLowerCase())
            })
        }
    }
}
